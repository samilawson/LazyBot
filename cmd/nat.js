...
exports.run = (bot, msg, params = []) => {
  ...
  const time = moment(now).format("H:mm:ss");
   const result = request.get(`https://www.nationstates.net/cgi-bin/api.cgi?nation=${name}&q=name+gdp+population+currency+animal+region+wa+flag+freedom+fullname+motto+influence+census;mode=score;scale=66`);
  ...
  result.then((res) => {
    parseString(res.text, (err, obj) => {
      embed.setColor(3447003)
        ...
        .addField('Population', (obj.NATION.POPULATION * 10000000).toLocaleString(), true)
        .addField('Economy', "$" + (obj.NATION.GDP).toLocaleString(), true)
        .addField('Currency', `${obj.NATION.CURRENCY}`, true)
        .addField('Freedom', obj.NATION.FREEDOM[0].CIVILRIGHTS + " | " + obj.NATION.FREEDOM[0].ECONOMY + " | " + obj.NATION.FREEDOM[0].POLITICALFREEDOM, true)
